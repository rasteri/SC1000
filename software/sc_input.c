...
#define BUTTONSTATE_WAITING 4
		...
		else // couldn't find input processor, just play the tracks
		{
			...
			//fscanf(fptr, "%f", &lasttargetpos, &lasttimestamp); // >= 0)
			//printf("%f %f\n", lasttargetpos, lasttimestamp);
			//fscanf(fptr, "%f", &lasttargetpos);
			...
			if (waitingToSend){
				//spin_lock(&deck[1].player.lock);
				if (fifoWrite(deck[1].player.scqueue, &sq)){
					waitingToSend = 0;
				}
				//spin_unlock(&deck[1].player.lock);
			}
